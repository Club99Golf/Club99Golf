...
export default function GolfApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      snap => {
        ...
        // Auto-expire: mark stale challenges in Firestore so they disappear for all users
        allItems.forEach(challenge => {
          if (challenge.status === "expired" || challenge.settled || challenge.status === "completed") return;
          ...
          if (expiresAt && expiresAt <= now) {
            updateDoc(doc(db, "challenges", challenge.id), { status: "expired" }).catch(() => {});
          }
        });

        const items = allItems.filter(challenge => {
  ...
  useEffect(() => {
    ...
    // Auto-switch to challenges tab ONLY when lobby is full and challenge is today
    const today = new Date().toISOString().slice(0, 10);
    const isReadyToPlay = availableChallenge &&
      (1 + (availableChallenge.joinedBy || []).length) >= Number(availableChallenge.maxPlayers || 2) &&
      availableChallenge.date === today;

    if (isReadyToPlay && !challengeAutoSwitchedRef.current && !liveRound) {
      challengeAutoSwitchedRef.current = true;
      setActiveChallengeId(availableChallenge.id);
      setTab("profile");
      setFlash({ type: "up", msg: "🏌️ YOUR CHALLENGE IS LIVE — START YOUR ROUND!" });
      setTimeout(() => setFlash(null), 4000);
    }
    if (!availableChallenge) {
      challengeAutoSwitchedRef.current = false;
    }
  }, [authUser?.uid, challenges]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function submitLiveRound() {
    ...
    // Low GPS = 0 coins (must actually move to earn); daily limit = 25% after 2 rounds
    const rewardMultiplier = !gpsVerified ? 0 : dailyMultiplier;
    ...
    const rewardNote = !gpsVerified ? " · No coins — low GPS movement" : rewardMultiplier < 1 ? " · Daily limit" : "";
    setFlash({ type: ovrDelta >= 0 ? "up" : "down", msg: `OVR ${ovrDelta >= 0 ? "+" : ""}${ovrDelta}  ·  ${!gpsVerified ? "0" : `+${coinsEarned}`} 🪙${rewardNote}` });
    setTab("profile");
        ...
        recordChallengeScore(matchingChallenge.id, authUser.uid, profile.username, s).then(result => {
          ...
          if (result.settled) {
            ...
            if (result.winner?.uid === authUser.uid && (result.payout || result.wager) > 0) {
              const winAmount = Number(result.payout || result.wager || 0);
              setTimeout(() => setFlash({ type: "up", msg: `🏆 YOU WON! +${winAmount.toLocaleString()} 🪙 CHALLENGE COMPLETE` }), 600);
            }
          } else {
  ...
  const subPerks = {
    free:  { crewCost: 1000, challengeFee: 500,  monthlyCoins: 0,   attrDetails: false, proItems: false, proBadge: false },
    basic: { crewCost: 500,  challengeFee: 300,  monthlyCoins: 300, attrDetails: true,  proItems: false, proBadge: false },
    pro:   { crewCost: 0,    challengeFee: 0,    monthlyCoins: 700, attrDetails: true,  proItems: true,  proBadge: true  },
  }[subscription] || { crewCost: 1000, challengeFee: 500, monthlyCoins: 0, attrDetails: false, proItems: false, proBadge: false };
  const CHALLENGE_FEE = subPerks.challengeFee;
  const CREW_COST = subPerks.crewCost;
