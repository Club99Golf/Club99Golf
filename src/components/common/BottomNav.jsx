export default function BottomNav(props) {
  ...
  return (
      <nav className="bottom-nav" style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", borderTop: "1px solid #e5e7eb", display: (tab === "profile" && liveRound) ? "none" : "flex", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom)" }}>
        {[
          ...
          { id: "leaderboard", label: "Social", icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="18" y="3" width="4" height="18" rx="1"/><rect x="10" y="8" width="4" height="13" rx="1"/><rect x="2" y="13" width="4" height="8" rx="1"/></svg> },
          ...
          <button key={id} onClick={() => { setTab(id); setFlash(null); setBadgeFlash(null); }} style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "8px 2px 10px", border: "none", background: "transparent", color: tab === id ? Theme.primaryGreen : "#374151", cursor: "pointer", position: "relative", transition: "color 0.15s" }}>
            {icon}
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: 0.3, marginTop: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}>{label}</span>
