  ...
  return (
        ...
        <div className="tab-scroll" style={{ paddingBottom: 80 }}>
          ...
          {challengesLoading ? (
            ...
            // Show all non-expired challenges plus completed ones up to 2 days after challenge date
            if (c.status === "expired") return false;
            if (c.settled || c.status === "completed") {
              const challengeDate = c.date ? new Date(c.date) : null;
              if (!challengeDate) return true;
              const twoDaysLater = new Date(challengeDate);
            ...
            <div style={{ padding: "0 16px" }}>
              {challenges.filter(c => {
                ...
                <ChallengeCard key={c.id} challenge={c} myUid={authUser?.uid} myUsername={profile.username} myCoins={profile.coins || 0} challengeFee={CHALLENGE_FEE}
                  onJoin={async (id, wager) => {
                    const result = await joinChallengeInDb(
                      id,
                      authUser.uid,
                      profile.username,
                      profile.ovr,
                    ...
                    if (result === true) {
                      ...
                      // Deduct subscription-based entry fee + wager from local profile immediately
                      setProfile(p => ({ ...p, coins: Math.max(0, (p.coins || 0) - CHALLENGE_FEE - Number(wager || 0)) }));
                    }
                    return result;
                  }}
        ...
        {showChallengeModal && (() => {
          ...
          return (
            <div onClick={closeModal} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", zIndex: 300 }}>
              <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxHeight: "92vh", display: "flex", flexDirection: "column", boxSizing: "border-box", paddingBottom: "env(safe-area-inset-bottom)" }}>
                  ...
                  <>
                    <div style={{ padding: "16px 16px 0", flexShrink: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                        <div>
                          <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Bebas Neue", letterSpacing: 1, lineHeight: 1 }}>POST A CHALLENGE</div>
                    ...
                    <div style={{ overflowY: "auto", WebkitOverflowScrolling: "touch", flex: 1, padding: "0 16px 24px" }}>
                        ...
                        {[2, 3, 4, 5, 6, 7, 8].map(n => {
                          ...
                          return (
                            <button
                              ...
                              style={{ flex: 1, aspectRatio: "1", maxWidth: 42, borderRadius: "50%", border: sel ? `2.5px solid ${Theme.primaryGreen}` : "2px solid #e5e7eb", background: sel ? "rgba(125,162,126,0.1)" : "#f9fafb", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", outline: "none", padding: 0 }}
                            >
                              <span style={{ fontSize: 15, fontWeight: 900, fontFamily: "Bebas Neue", color: sel ? Theme.primaryGreen : "#374151", lineHeight: 1 }}>{n}</span>
                            </button>
                          );
                        })}
                      </div>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 6 }}>
                        {["A","B","C","D"].map(team => {
                          ...
                          return (
                            <div key={team} style={{ background: isEmpty ? "#f9fafb" : bg, borderRadius: 12, padding: "8px 6px", minHeight: 68, border: isEmpty ? "1.5px dashed #e5e7eb" : `1.5px solid ${color}22`, transition: "all 0.2s" }}>
                              <div style={{ fontSize: 9, fontWeight: 800, color: isEmpty ? "#d1d5db" : color, letterSpacing: 1.2, textAlign: "center", marginBottom: 6 }}>TEAM {team}</div>
                                ...
                                {teamSlots.map(({ i }) => {
                                  ...
                                  return (
                                    <button
                                      key={i}
                                      onClick={() => cycleTeam(i)}
                                      style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", cursor: "pointer", padding: 2 }}
                                    >
                                      <div style={{ width: 34, height: 34, borderRadius: "50%", background: isMe ? "#f3f4f6" : "#e5e7eb", border: `2px solid ${color}`, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                        {isMe && profilePic
                                          ? <img src={profilePic} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                                          : <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>}
                                      </div>
                                      <span style={{ fontSize: 8, fontWeight: 700, color, maxWidth: 38, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{isMe ? (profile.username || "You") : `P${i + 1}`}</span>
                                    </button>
                                  );
                                })}
                              </div>
                      ...
                      <div style={{ fontSize: 10, color: "#9ca3af", textAlign: "center", marginBottom: 16 }}>Tap a player bubble to move them to the next team</div>
                    ...
                    <div style={{ padding: "16px 16px 0", flexShrink: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <div>
                            <div style={{ fontSize: 18, fontWeight: 900, fontFamily: "Bebas Neue", letterSpacing: 1, lineHeight: 1 }}>DETAILS</div>
